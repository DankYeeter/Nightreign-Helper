...
| SEC-011 | `pip-audit` fehlt; Schwachstellenstand der Abhaengigkeiten ungeprueft | Niedrig | offen - **sperrt nicht** | 2026-09-02 |
...
| SEC-016 | `dcx.py:38-41` reicht `uncompressed_size` ungeprueft als `max_output_size` an `zstandard` | Niedrig | offen - **Vorabbelegung gemessen bestaetigt** (256 MiB Deckel -> 256 MiB Haufen bei 1 KiB Ergebnis). Prioritaet bleibt Mittel, kein Blocker | 2026-09-02 |
| SEC-017 | `tpf.read` prueft `file_count` gegen nichts; quadratische Speicheraufnahme, 256 KiB Eingabe -> 3,2 GiB Spitzenhaufen gemessen (`tpf.py:27/37/53`) | Niedrig | **neu, offen** | 2026-09-02 |
| SEC-018 | `dcx.py:41-42` DFLT-Zweig ohne jeden Deckel, `zlib.decompress` ohne `bufsize`; x1029 gemessen. Gehoert in denselben Fix wie SEC-016 | Niedrig | **neu, offen** | 2026-09-02 |
...
| SEC-020 | PowerShell-Pfad wird aus `%SystemRoot%` abgeleitet; Umgebungs-Hijack ueber `HKCU\Environment` bleibt (`shortcut.py:68`) | Niedrig | **neu, offen** - Haertung, keine Rechteausweitung | 2026-09-02 |
