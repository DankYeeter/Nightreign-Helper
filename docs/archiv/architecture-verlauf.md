# Architektur — vollstaendiger Verlauf

*Dies ist `ARCHITECTURE.md` in der Fassung vom 12.09.2026, **unveraendert und
in urspruenglicher Reihenfolge**. Kein Wort ist gestrichen, kein Wort ist
umgestellt.*

**Warum es diese Datei gibt.** `ARCHITECTURE.md` war am 12.09.2026 auf 5972
Zeilen und 21 H2-Abschnitte gewachsen, elf davon Nachtraege. Wer den heute
geltenden Vertrag einer Entscheidung suchte, musste bis zu drei Abschnitte
lesen und selbst entscheiden, welcher gewinnt (AD-028 wird in Nachtrag VIII,
IX und X nachgetragen). Seit T-182 gilt die Arbeitsteilung:

| Datei | Frage, die sie beantwortet |
|---|---|
| `ARCHITECTURE_REGISTER.md` | Welche Fassung gilt heute, und welche offene Frage ist noch offen? |
| `ARCHITECTURE.md` | Was gilt — nach Themenbereich geordnet. |
| `docs/archiv/architecture-verlauf.md` (diese Datei) | Wie es dazu kam — chronologisch, mit allen Begruendungen und verworfenen Alternativen. |

**Der Wert dieser Datei sind die verworfenen Alternativen.** Eine Entscheidung
ohne die Optionen, gegen die sie getroffen wurde, ist keine Entscheidung,
sondern eine Behauptung. Wer eine Entscheidung revidieren will, liest **hier**
nach, was schon einmal geprueft und warum es verworfen wurde.

**Zeilennummern.** Alle Fundstellen in `ARCHITECTURE_REGISTER.md` beziehen
sich auf die urspruengliche Zaehlung. In dieser Datei liegen sie um die Laenge
dieses Kopfes hoeher; massgeblich ist immer die genannte Ueberschrift.

## Inhaltsverzeichnis

- **Gemessene Grundzahlen**  *(urspr. Zeile 13)*
- **Überblick**  *(urspr. Zeile 113)*
- **Wichtige Korrektur zur Auftragslage (erledigt)**  *(urspr. Zeile 148)*
- **Teil 1 — Strukturaudit des Bestands**  *(urspr. Zeile 163)*
  - Was trägt  *(urspr. Zeile 165)*
  - Befunde, priorisiert  *(urspr. Zeile 187)*
- **Teil 2 — Struktur des Build-Beraters**  *(urspr. Zeile 209)*
  - Modulschnitt  *(urspr. Zeile 211)*
  - Wo Zustand liegt  *(urspr. Zeile 253)*
  - Datenfluss eines Laufs  *(urspr. Zeile 267)*
- **Entscheidungen**  *(urspr. Zeile 293)*
  - AD-001 — Der Berater wird ein eigenes Paket, nicht ein Modul und nicht Teil von `app.py` (2026-09-01, Status: aktiv)  *(urspr. Zeile 295)*
  - AD-002 — `model.compute()` ist die einzige Bewertungsautorität; es gibt keinen zweiten, schnelleren Scorer (2026-09-01, Status: aktiv)  *(urspr. Zeile 331)*
  - AD-003 — Beam-Suche über die Slots, nicht Vollprodukt, nicht Greedy, nicht Solver (2026-09-01, Status: aktiv)  *(urspr. Zeile 376)*
  - AD-004 — Zielrichtungen als Registry reiner Funktionen `Build -> GoalScore`, mit ausdrücklicher Unwissensliste (2026-09-01, Status: aktiv)  *(urspr. Zeile 476)*
  - AD-005 — Die Attack-Rating-Rechnung wird aus `app.py` nach `nrplanner/damage.py` gezogen, bevor der Berater gebaut wird (2026-09-01, Status: aktiv)  *(urspr. Zeile 621)*
  - AD-006 — Hintergrundlauf über `QThread` + Worker-Objekt + Signale, mit kooperativem Abbruch (2026-09-01, Status: aktiv)  *(urspr. Zeile 660)*
  - AD-007 — Ergebnis-Cache nur im Speicher (LRU), nichts auf Platte (2026-09-01, Status: aktiv)  *(urspr. Zeile 753)*
  - AD-008 — Das Suchproblem wird über die kanonisierte Slot-Farbmenge geschlüsselt, nicht über die Gefäss-Id (2026-09-01, Status: aktiv)  *(urspr. Zeile 802)*
  - AD-009 — Testsockel headless, ohne neue Laufzeit-Dependency (2026-09-01, Status: aktiv; Werkzeug auf `pytest` geändert, siehe Nachtrag)  *(urspr. Zeile 839)*
  - AD-010 — Die Unwissensliste ist Teil des Ergebnisses, nicht eine Fussnote in der Oberfläche (2026-09-01, Status: aktiv)  *(urspr. Zeile 914)*
  - AD-011 — Prüfvokabular als freie Funktionen in `binary.py`, nicht als Methoden der `Reader`-Klasse (2026-09-01, Status: aktiv, vom `director` angenommen)  *(urspr. Zeile 958)*
  - AD-012 — Kein `defusedxml`; stattdessen Grössendeckel vor dem Parsen (2026-09-01, Status: aktiv, vom `director` angenommen samt Neubewertungs-Bedingung)  *(urspr. Zeile 1046)*
  - AD-013 — Ein Vorschlag ist eine Menge von Handles, nicht von Rollen; ein belegtes Exemplar fällt aus dem Kandidatenraum (2026-09-01, Status: aktiv)  *(urspr. Zeile 1112)*
  - AD-014 — Ein festgehaltener Slot ist Randbedingung der Suche, nicht Startwert: er geht als Grundzustand in jede Bewertung ein (2026-09-02, Status: aktiv)  *(urspr. Zeile 1190)*
  - AD-015 — Flüche gehen als gewöhnliche Effekte in dieselbe `compute()`-Bewertung; ausgewiesen werden sie aus `Build.sources`, nicht aus einer zweiten Rechnung (2026-09-02, Status: aktiv)  *(urspr. Zeile 1355)*
  - AD-016 — Der Haltezustand geht in die Kanonisierung, den Cache-Schlüssel und den Generationszähler ein (2026-09-02, Status: aktiv; präzisiert AD-006, AD-007, AD-008)  *(urspr. Zeile 1421)*
  - AD-017 — Der Haltezustand gehört zum Paar (Nightfarer, Gefäss) und lebt im Fenster, nicht auf Platte (2026-09-02, Status: aktiv; präzisiert AD-014.8 und die Nicht-tun-Regel 15)  *(urspr. Zeile 1474)*
  - AD-018 — Der Hauptweg des Beraters ist der Grenzbeitrag je Kandidat im Picker; er ist dieselbe Rechnung wie die Vorsortierung, und der Gesamtlauf bleibt als zweite Frage bestehen (2026-09-02, Status: aktiv in der Sache; **Punkt 4 und die Laufzeittabelle nachgezogen durch AD-028**, 2026-09-08)  *(urspr. Zeile 1547)*
  - AD-019 — Eine gemeinsame Fassade über beiden Rechenschichten, kein zweiter Wächter über `weapons.rate` (2026-09-02, Status: aktiv; erweitert AD-005, Vorbedingung für AD-018)  *(urspr. Zeile 1687)*
  - AD-020 — Was die Fassade ausdrücklich NICHT vereinheitlicht (2026-09-02, Status: aktiv; Randbedingung von AD-019)  *(urspr. Zeile 1902)*
  - AD-021 — Der Wächter sichert nicht „ein Aufrufer", sondern „nur die Fassade rechnet"; dasselbe Werkzeug, zwei Zusicherungen (2026-09-02, Status: aktiv; erweitert AD-002 und `test_one_build.py`)  *(urspr. Zeile 1974)*
  - AD-022 — Ein Name je Schicht: `scaled_*` vor der Multiplikatorschicht, `final_*` danach; die Umbenennung ist ein eigener Schritt W1b vor W2 (2026-09-02, Status: aktiv; präzisiert AD-019)  *(urspr. Zeile 2039)*
  - AD-023 — Das Invarianzargument aus Nachtrag III gilt nur für Multiplikatoren des Grundzustands; für Kandidaten, die selbst eine Angriffsrate mitbringen, kann W6 die Reihenfolge drehen (2026-09-02, Status: aktiv; **korrigiert Nachtrag III**)  *(urspr. Zeile 2107)*
  - AD-024 — Summationsreihenfolge ist eine Eindeutigkeits-, keine Genauigkeitsentscheidung; sie wird nur geändert, wo sie zwei Darstellungen derselben Zahl beseitigt (2026-09-02, Status: aktiv; folgt aus Zusicherung Z1 in AD-019, ergänzt AD-020)  *(urspr. Zeile 2188)*
- **Umsetzung — Schnitt in einzeln lauffähige Schritte**  *(urspr. Zeile 2292)*
  - Was der `developer` ausdrücklich nicht tun soll  *(urspr. Zeile 2323)*
- **Risiken und Prüfpunkte**  *(urspr. Zeile 2344)*
- **Bewusst nicht getan**  *(urspr. Zeile 2360)*
- **Offene Fragen**  *(urspr. Zeile 2411)*
- **Nachtrag 2026-09-02 — Festhalten und Flüche (AD-014 bis AD-016)**  *(urspr. Zeile 2453)*
  - Änderungen am Umsetzungsschnitt  *(urspr. Zeile 2463)*
  - Prüfpunkte, Ergänzung zum Mindestumfang aus AD-009  *(urspr. Zeile 2477)*
  - Risiken, Ergänzung  *(urspr. Zeile 2500)*
  - Was der `developer` zusätzlich ausdrücklich nicht tun soll  *(urspr. Zeile 2510)*
  - Folgen für `UI_SPEC.md` — an den `ui-ux-designer`  *(urspr. Zeile 2526)*
  - Offene Fragen, neu  *(urspr. Zeile 2538)*
- **Nachtrag II 2026-09-02 — Antworten des Nutzers zu OF-12, OF-13, F2 (AD-017, AD-018)**  *(urspr. Zeile 2568)*
  - Änderungen am Umsetzungsschnitt (zusätzlich zu Nachtrag I)  *(urspr. Zeile 2582)*
  - Prüfpunkte, Ergänzung (zu AD-009 und Nachtrag I)  *(urspr. Zeile 2591)*
  - Risiken, Ergänzung  *(urspr. Zeile 2607)*
  - Was der `developer` zusätzlich nicht tun soll  *(urspr. Zeile 2616)*
  - Offene Frage, neu  *(urspr. Zeile 2629)*
- **Nachtrag III 2026-09-02 — Die zweite Rechenschicht (AD-019 bis AD-021, QA-058)**  *(urspr. Zeile 2641)*
  - Änderungen am Umsetzungsschnitt  *(urspr. Zeile 2648)*
  - Reihenfolge gegenüber dem Berater — die Frage des `director`  *(urspr. Zeile 2664)*
  - Prüfpunkte, Ergänzung  *(urspr. Zeile 2691)*
  - Risiken, Ergänzung  *(urspr. Zeile 2712)*
  - Was der `developer` zusätzlich ausdrücklich nicht tun soll  *(urspr. Zeile 2721)*
  - Offene Fragen, neu  *(urspr. Zeile 2742)*
- **Nachtrag IV 2026-09-02 — Antworten vor W2 (Z1, AD-022, AD-023)**  *(urspr. Zeile 2765)*
  - Auflagen für W2  *(urspr. Zeile 2771)*
  - Korrektur an Nachtrag III  *(urspr. Zeile 2799)*
  - Prüfpunkte, Ergänzung  *(urspr. Zeile 2815)*
  - Was der `developer` zusätzlich nicht tun soll  *(urspr. Zeile 2826)*
- **Nachtrag V 2026-09-02 — Die Klammerungsfrage aus W4 (AD-024)**  *(urspr. Zeile 2840)*
  - Die drei Antworten in Kurzform  *(urspr. Zeile 2846)*
  - Beide Stellen unter einer Regel  *(urspr. Zeile 2868)*
  - Prüfpunkte, Ergänzung  *(urspr. Zeile 2881)*
  - Was der `developer` zusätzlich nicht tun soll  *(urspr. Zeile 2892)*
- **Nachtrag VI 2026-09-05 — Zwei Klassen von Vorbehalten, die Ergebnisform des Pickers, und vier Präzisierungen (AD-025)**  *(urspr. Zeile 2905)*
  - AD-025 — Ein Vorbehalt gehört entweder der Registry oder dem Ergebnis, und die Frage „kann der Satz geschrieben werden, bevor der Lauf bekannt ist?" entscheidet, welchem (2026-09-05, Status: aktiv; präzisiert AD-004 und AD-010, Vorbedingung für S8/S9/S10)  *(urspr. Zeile 2921)*
  - Die Ergebnisform: was `SlotPool` bekommt und was ausdrücklich nicht  *(urspr. Zeile 3067)*
  - Präzisierung AD-004 — die `unknowns` einer Zielrichtung zerfallen, und die konditionale Zeile bekommt einen Ort (D2)  *(urspr. Zeile 3166)*
  - Präzisierung AD-010 — was „Pflichtfeld im Ergebnis" nach D1 heisst, und warum AK-50 nicht widerspricht  *(urspr. Zeile 3226)*
  - Präzisierung AD-016 — der Cache-Schlüssel ist positionsabhängig, und `held_fingerprint` entfällt (D3, QA-107)  *(urspr. Zeile 3257)*
  - Präzisierung AD-009 — die Nummer 18 war zweimal vergeben (D4)  *(urspr. Zeile 3342)*
  - Prüfpunkte, Ergänzung (zu AD-009, Nachträge I bis V)  *(urspr. Zeile 3386)*
  - Risiken, Ergänzung  *(urspr. Zeile 3427)*
  - Was der `developer` zusätzlich ausdrücklich nicht tun soll  *(urspr. Zeile 3439)*
  - Bewusst nicht getan, Ergänzung  *(urspr. Zeile 3464)*
  - Offene Fragen, neu  *(urspr. Zeile 3483)*
- **Nachtrag VII 2026-09-07 — Was `not_counted` zählt, wenn die Schranke eine Waffe ist (AD-026), und zwei Korrekturnotizen**  *(urspr. Zeile 3520)*
  - 0. Messumgebung, für jede Zahl dieses Nachtrags (L-001, L-009)  *(urspr. Zeile 3530)*
  - AD-026 — Eine unerfüllte Waffentyp-Schranke ist eine Bedingung, in die der Spieler sich versetzen kann, und bleibt in `Build.situational`; falsch ist nicht die Rechnung, sondern der Docstring (2026-09-07, Status: aktiv; präzisiert AD-010, bestätigt D-3)  *(urspr. Zeile 3584)*
  - Was der `developer` zu tun hat (Fixrichtung, kein Patch)  *(urspr. Zeile 3743)*
  - AK-167 und AK-168 — nachgeprüft, nicht übernommen  *(urspr. Zeile 3797)*
  - Korrekturnotiz zu AD-015 (2026-09-07; Herkunft: Entscheidung des `director` vom 06.09.2026, umgesetzt in `explain.py` seit T-081)  *(urspr. Zeile 3840)*
  - Korrekturnotiz zu AD-003.5 (2026-09-07; Herkunft: D-5 des `director` vom 06.09.2026, gemeldet vom `developer` in T-067)  *(urspr. Zeile 3878)*
  - Bewusst nicht getan, Ergänzung  *(urspr. Zeile 3913)*
  - Offene Fragen, neu  *(urspr. Zeile 3938)*
- **Nachtrag VIII 2026-09-08 — Der Berater rechnet im Hauptthread, und das Lesen des Spielstands auch (AD-028, AD-029)**  *(urspr. Zeile 3962)*
  - AD-028 — Der Picker-Weg verlaesst den Hauptthread ueber eine **zweite Instanz derselben** `AdvisorController`-Klasse, nicht ueber die vorhandene und nicht ueber einen eigenen Weg (2026-09-08, Status: aktiv **in der Sache**; **Punkte 1 bis 4, W3 und die Schritte U5a/U5b/U6/U7 nachgezogen durch Nachtrag IX**, 2026-09-08; **W1 und der Vertrag des `AdvisorController` nachgezogen durch Nachtrag X**, 2026-09-08; zieht AD-018 Punkt 4 nach, praezisiert AD-006 und AD-007)  *(urspr. Zeile 3985)*
  - AD-029 — Das Lesen des Spielstands wird **zuerst am Lesen selbst** repariert; die Verlagerung in einen Worker ist eine zweite Stufe, die an eine Messung mit benanntem Ausloeser haengt (2026-09-08, Status: aktiv; beruehrt AD-006.7, AD-006.8, SEC-022)  *(urspr. Zeile 4213)*
  - Umsetzung — Schnitt in einzeln lauffaehige Schritte (Nachtrag VIII)  *(urspr. Zeile 4377)*
  - Risiken und Pruefpunkte, neu  *(urspr. Zeile 4408)*
  - Bewusst nicht getan, neu  *(urspr. Zeile 4420)*
  - Offene Fragen, neu  *(urspr. Zeile 4440)*
- **Nachtrag IX 2026-09-08 — Der Zielrichtungswechsel war nie eine Frage (Nachtraege zu AD-028, Fassung 2 von U5a bis U7)**  *(urspr. Zeile 4475)*
  - IX-0 — Die Nachpruefung: der Befund haelt, an einer Stelle weiter als behauptet, an einer anderen nicht  *(urspr. Zeile 4494)*
  - IX-1 — Die Entprellung der Picker-Spur ist **0 ms**, und eine bereits bekannte Antwort wird **sofort** zurueckgegeben, nicht ueber den Zeitgeber  *(urspr. Zeile 4542)*
  - IX-2 — Die Picker-Spur fragt unter einer **kanonischen** Zielrichtung; die gewaehlte Richtung ist Sache der Anzeige  *(urspr. Zeile 4651)*
  - IX-3 — Die 64 bleibt stehen, ihre Herleitung deckt die neue Antwortform nicht; U7 misst nach  *(urspr. Zeile 4714)*
  - IX-4 — AD-028 Punkt 4 nachgezogen, W3 neu gegruendet, W4 und W5 dazu  *(urspr. Zeile 4763)*
  - IX-5 — Vorwaermen: **Backlog-Vorschlag**, weder Ersatz fuer AD-028 noch Zusatz zu ihm  *(urspr. Zeile 4825)*
  - Umsetzung — Fassung 2 von U5a bis U7 (ersetzt die entsprechenden Zeilen in Nachtrag VIII)  *(urspr. Zeile 4876)*
  - Risiken und Pruefpunkte, neu (zu den Zeilen aus Nachtrag VIII)  *(urspr. Zeile 4908)*
  - Bewusst nicht getan, neu (zu Nachtrag VIII)  *(urspr. Zeile 4920)*
  - Offene Fragen, neu  *(urspr. Zeile 4935)*
- **Nachtrag X 2026-09-08 — Was der Vertrag des `AdvisorController` wirklich zusagt (drei Nachtraege zu AD-028, **keine neue AD-Nummer**)**  *(urspr. Zeile 4960)*
  - X-0 — Die Zusage, praezise: sie gilt fuer die Frage, die beim Ende noch die aktuelle ist  *(urspr. Zeile 4994)*
  - X-1 — `shutdown()`: der Text wird praezisiert, **und** das Schweigen wird zur Eigenschaft gemacht (Punkt 1)  *(urspr. Zeile 5046)*
  - X-2 — `search.Cancelled` ist kein Ausgang, sondern eine Quittung — und „haelt ueber drei Aufrufer" ist **keine** bewachbare Zusage (Punkt 2)  *(urspr. Zeile 5116)*
  - X-3 — W1 erlaubt drei Aufrufstellen: **so ist es gemeint.** Der Wortlaut von AD-028 wird nachgezogen (Punkt 3)  *(urspr. Zeile 5195)*
  - Meldung an den `director`: **AK-218 ist im Wortlaut enger als der gebaute Zustand** — `UI_SPEC.md` habe ich nicht angefasst  *(urspr. Zeile 5258)*
  - Umsetzung — U9 und U10 (zu den Schritten aus Nachtrag VIII und IX)  *(urspr. Zeile 5283)*
  - Risiken und Pruefpunkte, neu (zu Nachtrag VIII und IX)  *(urspr. Zeile 5327)*
  - Bewusst nicht getan, neu (zu Nachtrag VIII und IX)  *(urspr. Zeile 5338)*
  - Offene Frage, neu  *(urspr. Zeile 5354)*
- **Nachtrag XI 2026-09-08 — Der Erststart bekommt einen Weg von Hand (AD-030), und der Vorfilter faellt zurueck statt zu verweigern (AD-031)**  *(urspr. Zeile 5370)*
  - XI-0 — Was ich am Bestand nachgeprueft habe, statt es zu uebernehmen  *(urspr. Zeile 5391)*
  - AD-030 — Der gemerkte Pfad lebt in **zwei festen Schluesseln** des vorhandenen Speichers, und das Programm hat **einen** Ort, an dem es den Spielordner aufloest (2026-09-08, Status: aktiv; benennt die Reichweite von OF-15, beruehrt AD-013, AK-107, AK-117, AK-121, AK-125)  *(urspr. Zeile 5420)*
  - AD-031 — Bricht die Id-Annahme, **waehlt** der Scan den langsamen Weg, statt zu verweigern; der langsame Weg wird dafuer wieder gebaut (2026-09-08, Status: aktiv; **loest AD-029 Vertrauensgrenze Punkt 3 ab**)  *(urspr. Zeile 5668)*
  - Umsetzung — Schnitt in einzeln lauffaehige Schritte  *(urspr. Zeile 5791)*
  - Regressionstests — dieselben fuer `developer` und `qa-engineer`  *(urspr. Zeile 5826)*
  - Was der `developer` ausdruecklich **nicht** tun soll  *(urspr. Zeile 5862)*
  - Risiken und Pruefpunkte  *(urspr. Zeile 5890)*
  - Bewusst nicht getan  *(urspr. Zeile 5902)*
  - Offene Fragen  *(urspr. Zeile 5929)*

---

*Ab hier: `ARCHITECTURE.md` im Wortlaut vom 12.09.2026, Zeile 1 bis 5972.*
