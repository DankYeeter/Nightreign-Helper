...
| AK-05 | Bei der abgeleiteten Startbreite (A14: Maximum aus Effekttabelle und Leistenbedarf im Vorschlagszustand), **ab 1536 px logischer Breite** (A34), gemessen am laufenden Fenster, ist in der Advisor bar kein Text abgeschnitten ausser der Statuszeile; die darf schrumpfen, ihr voller Text steht im Tooltip. Gilt in beiden Zustaenden (mit/ohne Vorschlag); unter 1536 px siehe AK-271. | A01 Z344 + A14 Z2555 (Startbreite abgeleitet statt 1320 px) + A31 + A32 (T-226) + A34 (Untergrenze, T-233) | T-277e, 2026-09-15 | entschieden A31 (13.09.2026) — 1320 px aufgegeben, nicht mehr widerspruechlich; A32 zieht A14 auf Zeilenbedarf nach; A34 setzt Untergrenze 1536 px (AK-271); T-277e: `Filters`-Knopf (AK-302) +86 px Leistenbedarf, `_opening_width()` unveraendert 1608 px, Boden haelt (72 px Marge, ungedeckelter Fall) |
...
| AK-182 | eigenes Element, zwei Eintraege. | A21 Z5755 | T-092, 2026-09-07 | **gegenstandslos (A18/T-248b) — es gibt keine `reading_box` mehr** |
| AK-183 | kein neuer Zustand. | A21 Z5765 | T-092, 2026-09-07 | **gegenstandslos (A18/T-248b) — kein Lesartwechsel mehr moeglich** |
| AK-184 | nie zwei Zahlen. | A21 Z5772 | T-092, 2026-09-07 | **gegenstandslos (A18/T-248b) — es gibt nur noch eine Rechnung** |
| AK-185 | genau viermal, nie je Zeile. | A21 Z5793 | T-092, 2026-09-07 | **gegenstandslos (A18/T-248b) — nichts benennt mehr eine Lesart** |
| AK-186 | Voreinstellung fuer Bedingungen, nie fuer Zahlen. | A21 Z5801 | T-092, 2026-09-07 | **gegenstandslos (A18/T-248b)** — die Abgrenzung „Berater vs. Werteblatt" lebt jetzt in AK-282 |
| AK-187 | die Lesarten teilen not_counted, sie erfinden nichts — Zahlen zuletzt auf 314 Kopien nachgezaehlt (176 + 27 = 203). | A21 Z5810 + A31 (312er-Nachtrag) + A33 (314er-Nachtrag), beide T-226/T-231 | T-231, 2026-09-13 | **gegenstandslos (A18/T-248b)** — `not_counted` ist keine Lesart-Aufteilung mehr, siehe AK-279 |
| AK-188 | die Listen behalten ihre Saetze — Zahlen zuletzt auf 314 Kopien nachgezaehlt (67/42/438 von 860/331). | A21 Z5818 + A31 (312er-Nachtrag) + A33 (314er-Nachtrag), beide T-226/T-231 | T-231, 2026-09-13 | **gegenstandslos (A18/T-248b)** — keine zwei Lesarten mehr, die verschiedene Zahlen zeigen koennten |
| AK-189 | der schlechteste Fall ist laenger, und das wird gemessen. | A21 Z5827 | T-092, 2026-09-07 | **gegenstandslos (A18/T-248b)** — es gibt keinen „schlechtesten Fall" (Lesart) mehr, nur noch Ausschluesse |
...
| AK-194 | die Statuszeile darf an der abgeleiteten Startbreite (wie AK-05), gemessen am laufenden Fenster, nie auf 0 px fallen (> 0 px); kein Kurzform-Rueckfall. | A21 Z5780 + A31 (Schwelle gesenkt) + A32 (Kurzform gestrichen, Bezugsbreite entschieden), beide T-226 | T-277e, 2026-09-15 | entschieden A32 (Nachtrag zu T-226, 13.09.2026) — nicht mehr widerspruechlich; T-277e: dieselbe Bezugsbreite (1608 px), unveraendert durch AK-302, siehe AK-05 |
...
| AK-268 | 4.8 (kein Spielstand) deaktiviert alle drei Bedienelemente der Zeile — Zielwahl, Lesart-Box und `Optimize`, nicht nur zwei; bestaetigt T-224s Bau | UI_SPEC §6.7 (neben AK-182) | T-226, 2026-09-13 | **gegenstandslos (A18/T-248b)** — es gibt nur noch zwei Bedienelemente (Zielwahl, `Optimize`), beide deaktiviert 4.8 unveraendert |
...
| AK-270 | QA-253: Abbruch eines Laufs durch Lesartwechsel sagt `The reading changed while this was working out — use Optimize again.`, nicht mehr den geteilten `Your build changed ...`-Satz | UI_SPEC §6.5/6.7 (neben AK-183) | T-231, 2026-09-13 | **entfallen ersatzlos (A18/T-248b)** — der Lesart-Umschalter ist gestrichen (AK-273-Vermerk), kein Lesartwechsel mehr moeglich; Prinzip fortgefuehrt in AK-289 |
...
| AK-273 | QA-261: `Worst case`/`Best case` lesen sich als Spielsituation, sind aber eine Rechenannahme; zwei Varianten (Umbenennung empfohlen, oder Erklaerungs-Tooltip) — **Wortwahl/Variante offen, Entscheidung beim App Designer**, nur das Geltungsbereich-Kriterium ist bindend | UI_SPEC §6.7 (neben AK-182/AK-185/AK-194) | T-247, 2026-09-14 | **gegenstandslos (A18/T-248b)** — es gibt keine Lesart mehr, ueber deren Namen zu entscheiden waere |
| AK-274 | QA-260: `SuggestionBlock`-Karte bekommt einen eigenen `Why`-Knopf neben `Use`, oeffnet denselben `WhyDialog` wie die Advisor bar; Sichtbarkeit an `ACTING_STATES` gekoppelt; 208 px/118 px (AK-46/AK-73) treffen nicht zu, anderes Widget | UI_SPEC §4.3 (nach AK-62) | T-247, 2026-09-14 | — |
| AK-275 | QA-265: Picker-Hauptsuchfeld (`relicpicker.py:1087`) bekommt den Platzhalter `Filter by effect — supports AND, OR, NOT and "quoted phrases"`, wortgleicher Suffix wie `arsenaltab.py:290`; `CustomRelicCard`s Teilstring-Suchfeld (Zeile 950) bleibt unberuehrt | UI_SPEC §5 (Ende, nach AK-263) | T-247, 2026-09-14 | — |
| AK-276 | Ort: Picker-Karte und `Why`-Dialog, an der Effekt-Id (nicht an der Zeile), drei Zustaende (neutral/Don't include/Must include, gegenseitig ausschliessend), ersetzt AD-035s reading-Mechanik in der Oberflaeche | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (Mechanismus entfaellt, Punkte in RelicCard/WhyDialog gestrichen, siehe §3.6/AK-301) |
| AK-277 | die Kontrolle ist der Aufzaehlpunkt selbst (kein Breitenzuwachs); drei Zustaende ueber bestehende Farben `ACCENT`/`BAD`/`MUTED`, keine neuen Hex-Werte; Tooltip je Zustand, zweite Legendenzeile im `Why`-Dialog | UI_SPEC §6.8 | T-248, 2026-09-14 | Tooltip-Wortlaut im ausgeschlossenen Zustand korrigiert (Nachtrag T-251c, Zyklus fuehrt zu Must include statt „normal zaehlend", DR-026); T-277a (Mechanismus entfaellt, siehe §3.6/AK-301) |
| AK-278 | Tastaturbedienung/Fokusring der Kontrolle wie jede andere Beraterhandlung (AK-25/AK-26); Zahl der Tab-Stopps je Karte wird beim ersten Bau gemessen | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (Mechanismus entfaellt, siehe §3.6/AK-301) |
| AK-279 | die 4.9b-Liste (`not_counted`) bekommt zwei Verwaltungslisten `Effects you've excluded:`/`Effects you require:`, jede Zeile mit derselben Kontrolle — der Rueckweg, ohne zur urspruenglichen Karte zurueckzumuessen | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (Listen bleiben, aber nur noch lesend/ohne Rueckweg-Klick, Ueberschriften umbenannt, siehe §3.6/AK-300/AK-301) |
| AK-280 | Zaehler nur im Zeilen-Tooltip (`bar.toolTip()`), kein neues Bedienelement, keine neue Breitenanforderung; sichtbar in allen 14 Zustaenden aus §4, nicht nur in Vorschlagszustaenden | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (Zaehlerklausel wandert vom Zeilen-Tooltip zum neuen `Filters`-Knopf, siehe §3.6/AK-302) |
| AK-281 | unerfuellbarer Pflicht-Effekt nennt sich beim Namen (A7/A12), Wortvorschlag nach AK-265-Muster; ob 4.10/4.11 oder ein eigener Zustand traegt, entscheidet der `architect` (AD-036) | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (Wortlaut `required`→`favourited`, siehe §3.6/AK-300) |
| AK-282 | A12-Abgrenzung: Markierung wirkt nur auf `AdvisorRequest`/Berater, nicht auf das Werteblatt und seine `declared`-Schalter (`statsheet.py`); Nicht-Ziel bestaetigt (keine Reliktsperrliste, kein Klassen-Schalter) | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (unveraendert inhaltlich, ausdruecklich auf das neue Fenster erweitert, siehe §3.6/AK-310) |
| AK-283 | Persistenz ist sichtbares Verhalten (Neustart zeigt dieselben Markierungen ohne Zutun); Speicherschema entscheidet AD-036 | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (gilt jetzt fuer Filterfenster/Why-Dialog/Filters-Tooltip statt Picker-Karte, siehe §3.6/AK-311) |
| AK-284 | A8 — alle neuen Zeichenketten aus AK-276 bis AK-283 Englisch, `html.escape`/`setTextFormat`-Disziplin wie AK-29/AK-30/AK-53 | UI_SPEC §6.8 | T-248, 2026-09-14 | T-277a (A8 gilt unveraendert auch fuer die neuen AK-300ff-Zeichenketten) |
| AK-285 | AK-05/AK-194 live neu vermessen ohne `reading_box`: Zeilenminimum sinkt 93 px (689→596 mit Aktionsknoepfen, 431→338 ohne), `Planner._opening_width()` bleibt bei **1608 px** unveraendert (Effekttabelle bindet, nicht die Leiste); AK-05/AK-269/AK-271 gelten unveraendert fort | UI_SPEC §6.8 | T-248, 2026-09-14 | — |
| AK-286 | Zweihandwert als vierte Form? Nein — Zusatz `{Form} {1H} / {2H} 2H` an den drei bestehenden AK-31-Formen, nur wo die Waffe zweihaendig fuehrbar ist; AK-32 gilt fort (dieselbe Beschriftung deckt beide Werte) | UI_SPEC §6.9 (A20) | T-248, 2026-09-14 | — |
| AK-287 | Platzbudget an allen vier Flaechen (Arsenal 200 px/AK-33, Reliktkarte 208 px/AK-73, Werteblatt, Berater-Zeile/AK-285) nach dem Bau live nachzumessen — kein Fensterlauf moeglich, solange die Flaeche nicht existiert | UI_SPEC §6.9 (A20) | T-248, 2026-09-14 | — |
| AK-288 | A7/A12: keine Zweihandregel treffend → unveraenderte Einhandform (kein `—`); Regel trifft, Spieldaten liefern keinen Modifikator → Satz statt Zahl; `when Two-Handing`-Effekte rechnen auf den Zweihandwert; drei Ranking-Optionen benannt, **Entscheidung offen beim App Designer** | UI_SPEC §6.9 (A20) | T-248, 2026-09-14 | **AK-293** (T-254a, 2026-09-15) — die offene Frage ist entschieden: Umschalter 1H/2H (AK-292) statt automatischer Wahl, dritte Option (Maximum) entfaellt |
| AK-289 | OF-39: Markieren einer Effektmenge (`Don't include`/`Must include`) waehrend laufendem `Optimize` bricht den Lauf ab (derselbe Pfad wie AK-12/AK-183), Statuszeile sagt eigenen Satz `The effects you marked changed while this was working out — use Optimize again.` statt des 4.7-Satzes — Wortlaut-Grundsatz des entfallenen AK-270 fortgefuehrt, keine neue Zustandsnummer | UI_SPEC §6.8 (neben AK-276/AK-281) | T-248b, 2026-09-14 | — |
| AK-290 | Nachtrag zu AK-276/277/289: Why-Zeile ausgeschlossen bestaetigt `{name}: you excluded it, so it is not counted.` (AD-036-Platzhalter in `explain.py` wird final); neu `{name}: you required it, so it always counts.` fuer die Pflicht-Zeile; Dreizustands-Legende `▲ marks an effect you required, a struck-through effect one you excluded; every other line counts as usual.` vervollstaendigt AK-277s zweistaatigen Entwurf | UI_SPEC §6.8 (neben AK-276/AK-277/AK-289) | T-248b, 2026-09-14 | Codename in Klammern veraltet: `_excluded_line` heisst seit T-250a (`115dd4e`) `_marked_line` — korrigiert (Nachtrag T-251c), Wortlaut unveraendert. Nachtrag T-254a: GOAL A18s Bedingungssatz gilt als erfuellt durch den Effektnamen selbst (Nutzerentscheidung), beruehrt AK-290s eigenen Wortlaut nicht; T-277a (Wortlaut `excluded`/`required`→`avoided`/`favourited`, Legende komplett ersetzt, siehe §3.6/AK-300/AK-301) |
| AK-291 | Nachtrag zu AK-281: Kombinations-Satz `"No combination of the copies you own carries {namen}, which you marked as required — no suggestion can meet that."` final, `{namen}` alphabetisch statt nach Effekt-Id sortiert (heutiger Bau sortiert nach Id); Zustand bleibt 4.11 (`SUGGESTED_WITH_AN_EMPTY_SLOT`), keine eigene Nummer — Ursache steht im `Why`-Footer, nicht in der Statuszeile | UI_SPEC §6.8 (neben AK-281/AK-289/AK-290) | T-248b, 2026-09-14 | T-277a (Wortlaut `required`→`favourited`, siehe §3.6/AK-300) |
| AK-292 | Umschalter `1H`/`2H` je Build (nicht je Kachel), Voreinstellung 1H (AN-2), zwischen Sechs-Kachel-Raster und AR-Bildunterschrift (AK-35), `QToolButton` im Hold/Held-Muster (AK-54), Tooltip nennt Wirkung + Persistenz | UI_SPEC §6.10 (A20, AN-2, OF-41) | T-254a, 2026-09-15 | — |
...
| AK-297 | QA-274: Markierungslegende (`MARK_LEGEND`, Wortlaut neu) steht immer sichtbar im Picker (neue Zeile in der `sorting`-Reihe) und im Why-Dialog (Sichtbarkeitsbedingung entfaellt) statt nur im Tooltip bzw. nur nachdem schon markiert wurde; `CARD_WIDTH_FLOOR = 208` bleibt unberuehrt | UI_SPEC Nachtrag T-267a (neben §6.8/AK-277) | T-267a, 2026-09-15 | T-277a (Mechanismus entfaellt — Legende betraf den gestrichenen Klick-Zyklus, siehe §3.6/AK-301) |
| AK-298 | QA-275: die per Schalter (AK-292) gewaehlte Hand ist auf Waffenkachel, Werteblatt und Arsenal-Kachel hervorgehoben (fett/`ACCENT` fuer die aktive Haelfte, `MUTED` fuer die andere) statt beide Haelften gleich zu zeigen; AK-286/AK-293 bleiben unveraendert, kein Zahlenwert aendert sich; `HAND_TOOLTIP` bekommt einen Halbsatz dazu | UI_SPEC Nachtrag T-267a (neben §6.10/AK-292/AK-293) | T-267a, 2026-09-15 | — |
| AK-299 | QA-278: `displayed_hands`-Format erweitert auf `{1H} 1H / {2H} 2H` (statt `{1H} / {2H} 2H`) — beide Haende beschriftet, nicht nur die zweite; gilt fuer alle drei AK-286/AK-298-Flaechen; AK-298-Betonung umschliesst Zahl+Label je Hand als eine Einheit; `HAND_TOOLTIP` nennt `1H`/`2H` statt `one-handed`/`two-handed`; Arsenal-Kachel (`CARD_WIDTH=200`, AK-33) als reales Breitenrisiko benannt, `relicpicker.py` (208 px) nicht betroffen — zeigt keinen AR-Wert | UI_SPEC Nachtrag (ui-ux-designer, neben §6.10/AK-286/AK-298) | 2026-09-15 | — |
| AK-300 | GOAL A21: programmweite Begriffe `Favourite`/`Avoid` ersetzen `Must include`/`Don't include`; Tabelle der final geltenden Zeichenketten (Verwaltungslisten, Zaehlerklausel, Why-Zeilen, A7-Saetze); offene Frage: Namenskollision mit dem bestehenden Relikt-Favouriten (★, `favourites.py`, Farbe `#a86fe0`) | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-301 | Director-Nachtrag 15.09.2026 12:30: AK-276/277/278/297 gegenstandslos, `RelicCard` zeichnet wieder unmarkiert wie vor T-248; `WhyDialog` bleibt lesend (▲/Durchstreichung ohne `MarkButton`), Verwaltungslisten ohne Klick-Rueckweg, neue Legende verweist auf das Filterfenster | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-302 | Neuer, staendig sichtbarer `QPushButton("Filters")` in der Leiste, ausserhalb des AK-07-Drei-Knopf-Budgets, aktiv auch waehrend `Optimize`; Symbolwahl Text statt Icon begruendet; AK-280-Zaehlerklausel wandert vom Zeilen-Tooltip zu diesem Knopf; Breitenfolge an AK-05/AK-194/A14 real nachgemessen: Knopf 80 px, Leistenbedarf +86 px in jedem Zustand (338→424 Grundzustand gemessen, 596→682 Vorschlagszustand rechnerisch), `_opening_width()` unveraendert 1608 px (Tabelle bindet), 1536-px-Boden haelt mit 72 px Marge im ungedeckelten Fall — `room=1536`-Fall mit Knopf noch nicht nachgemessen (offen) | UI_SPEC §3.6 | T-277e, 2026-09-15 | — |
| AK-303 | `EffectFilterWindow(QDialog)`, modal wie `WhyDialog`/`RelicPicker`, Titel `Effect filters`, Startgroesse ca. 520×560 px (Schaetzwert), bewusst unabhaengig von AK-271s 1536-px-Untergrenze, passt auch bei 1366×768 vollstaendig auf den Schirm | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-304 | Eine Zeile je Effekt-/Fluch-Id ueber alle besessenen Reliktkopien (Vereinigung, nicht Picker-gefiltert), fuenf Spalten (Marker Favourite, Marker Avoid, Name, Typ, Copies); `Copies` zaehlt besessene Reliktkopien, ausdruecklich abgegrenzt von `effectstab.py`s andersdefinierter `Copies`-Spalte, eigene Definitionszeile ueber dem Raster | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-305 | Zwei unabhaengige, disjunkte `QCheckBox` je Zeile (Favourite/Avoid) statt Klick-Zyklus, ueber `EffectFilters.mark`; Farben `ACCENT`/`BAD` (nicht `FAVOURITE`, das Relikt-Stern-Token bleibt getrennt); Tastaturweg wie das entfallene AK-278, jetzt hier | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-306 | Suchfeld mit derselben Booleschen Syntax wie das Picker-Hauptfeld (AK-275: AND/OR/NOT/Phrasen), wortgleicher Platzhalter-Aufbau | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-307 | Immer sichtbare Zaehlerzeile `"{gezeigt} of {gesamt} effects"`, ergaenzt um `favourited`/`avoided`-Klauseln nach dem `_clauses`-Muster | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
| AK-308 | Voreinstellung alphabetisch nach Name (wie AK-291s Begruendung), `QTableWidget.setSortingEnabled(True)` wie `effectstab.py`, jede der fuenf Spalten klickbar sortierbar | UI_SPEC §3.6 | T-277a, 2026-09-15 | — |
...
- **AK-05 — entschieden (A31).** Neue Bezugsbreite: die abgeleitete
  ...
  Vorschlagszustand), gemessen am laufenden Fenster; `1320 px` ist getilgt.
- **AK-194 — entschieden (A31 Schranke, A32 Kurzform + Bezugsbreite).**
