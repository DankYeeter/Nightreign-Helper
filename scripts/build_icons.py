"""Extract the icons the planner needs into a bundled pack.

Runtime extraction would mean decoding 4096x4096 BC7 atlases on every start,
so this is done once at build time and shipped inside the executable.
"""
...
from nrdata import (bnd4, dds, dvdbnd, extract, gamefiles, icons, oodle,
                    param, paramdef, regulation, tpf)

ROOT = pathlib.Path(__file__).resolve().parents[1]
DEFS = ROOT / "vendor" / "Paramdex" / "NR" / "Defs"
OUT = ROOT / "nrplanner" / "data" / "icons"
...
def main() -> None:
    ...
    for row in hero.rows:
        ...
        if image is None:
            print(f"   hero {row.id}: portrait {pid} missing")
            continue
    ...
    print(f"item icons: {len(manifest['items'])} of {len(wanted)} requested")
    ...
    print(f"boss icons: {len(manifest['menu'])} of {len(boss_icons)} requested")
    ...
    if DLC_SOLO_HEADER in arc.entries:
        ...
        print(f"DLC illustrations found: {len(dlc_entries)}")
    ...
    print(f"character variants: {total} across "
