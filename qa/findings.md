...
| QA-176 | **Ein per Subprocess gestartetes `run.py` kann unbegrenzt haengen, ohne je ein Fenster zu zeigen.** Vorgefunden: ein Prozess mit **0,00 s CPU, einem Thread, ueber 15 Minuten** am Leben, ohne Qt-Fenster — er erreicht die Einzelinstanz-Pruefung nie. Deckt sich mit einer Umgebungseigenschaft, die T-071 bereits gegen unveraendertes HEAD gegengeprueft hat, ist also **keine Regression**. Wirkung ist trotzdem real: dieser Zustand erzeugt genau das Bild "ich klicke und nichts passiert", das den Einzelinstanz-Schutz ueberhaupt noetig gemacht hat. Richtung: Zeitwaechter, der nach wenigen Sekunden ohne Fenster einen sichtbaren Fehler erzeugt statt still zu haengen | P3 | Major | developer | Prozesskennzahlen + Fensteraufzaehlung, T-076 | offen | 2026-09-06 |
